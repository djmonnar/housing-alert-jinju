...
진주 지역 청년/무주택자 주거 공고와 경남 소상공인·창업·마케팅 지원사업 공고를 매일 확인하고, 새 공고가 있으면 카카오톡 "나에게 보내기"로 알림을 보내는 Python 자동화 프로젝트입니다.
...
- 경남투자경제진흥원: 소상공인·마케팅·판로 지원사업
- 경남신용보증재단 소상공인종합지원: 소상공인 공고·교육·창업 지원
- 경남창업포털: 창업·스타트업 지원사업
- 경남테크노파크: 기업지원·사업화 지원사업
- 경상남도 해외마케팅 사업지원시스템: 해외마케팅·수출 지원사업
...
8. 경남 사업지원 공고는 별도 필터와 `data/seen_business_posts.json`으로 관리합니다.
9. 발송 성공 후에만 상태 파일에 저장합니다.
...
- `MAX_BUSINESS_ALERT_POSTS`: 한 번에 보낼 경남 사업지원 공고 최대 개수
...
카카오 기본 템플릿의 버튼 링크는 Product Link 웹 도메인 등록 조건을 탑니다. 그래서 공고 알림은 버튼에만 의존하지 않고, 공고별 텍스트 메시지 본문에 `원문: https://...` 형식으로 실제 URL을 직접 넣습니다. `KAKAO_WEB_LINK_URL`은 카카오 텍스트 템플릿의 보조 버튼 링크와 오늘의 주거 팁 참고 링크로 사용합니다.
...
- 경남 사업지원 공고 발송 여부는 `data/seen_business_posts.json`에 저장합니다.
